...
// OKExChainNet type specifies the type of OKExChain network
type OKExChainNet uint32

const (
	// MainNet is production network
	MainNet OKExChainNet = 66
	// TestNet is test network
	TestNet OKExChainNet = 65
)
...
// EXChainRPC is an interface to JSON-RPC eth service.
type EXChainRPC struct {
	*bchain.BaseChain
	client               *ethclient.Client
	rpc                  *rpc.Client
	timeout              time.Duration
	Parser               *OKExChainParser
...
// NewEXChainRPC returns new EthRPC instance.
func NewEXChainRPC(config json.RawMessage, pushHandler func(bchain.NotificationType)) (bchain.BlockChain, error) {
	var err error
	var c Configuration
	...
	s.Parser = NewOKExChainParser(c.BlockAddressesToKeep)
...
func (b *EXChainRPC) Initialize() error {
...
func (b *EXChainRPC) CreateMempool(chain bchain.BlockChain) (bchain.Mempool, error) {
	if b.Mempool == nil {
...
func (b *EXChainRPC) InitializeMempool(addrDescForOutpoint bchain.AddrDescForOutpointFunc, onNewTxAddr bchain.OnNewTxAddrFunc, onNewTx bchain.OnNewTxFunc) error {
	if b.Mempool == nil {
...
func (b *EXChainRPC) subscribeEvents() error {
	// subscriptions
...
func (b *EXChainRPC) subscribe(f func() (*rpc.ClientSubscription, error)) error {
	s, err := f()
	if err != nil {
		return err
	}
...
func (b *EXChainRPC) reconnectRPC() error {
...
func (b *EXChainRPC) Shutdown(ctx context.Context) error {
	b.closeRPC()
...
func (b *EXChainRPC) GetCoinName() string {
	return b.ChainConfig.CoinName
}

// GetSubversion returns empty string, ethereum does not have subversion
func (b *EXChainRPC) GetSubversion() string {
	return ""
}

// GetChainInfo returns information about the connected backend
func (b *EXChainRPC) GetChainInfo() (*bchain.ChainInfo, error) {
...
func (b *EXChainRPC) getBestHeader() (*ethtypes.Header, error) {
...
func (b *EXChainRPC) GetBestBlockHash() (string, error) {
	h, err := b.getBestHeader()
	if err != nil {
		return "", err
	}
...
func (b *EXChainRPC) GetBestBlockHeight() (uint32, error) {
	h, err := b.getBestHeader()
	if err != nil {
		return 0, err
	}
...
func (b *EXChainRPC) GetBlockHash(height uint32) (string, error) {
	var n big.Int
...
func (b *EXChainRPC) ethHeaderToBlockHeader(h *rpcHeader) (*bchain.BlockHeader, error) {
...
func (b *EXChainRPC) GetBlockHeader(hash string) (*bchain.BlockHeader, error) {
...
func (b *EXChainRPC) computeConfirmations(n uint64) (uint32, error) {
...
func (b *EXChainRPC) getBlockRaw(hash string, height uint32, fullTxs bool) (json.RawMessage, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
...
func (b *EXChainRPC) getERC20EventsForBlock(blockNumber string) (map[string][]*rpcLog, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
...
func (b *EXChainRPC) GetBlock(hash string, height uint32) (*bchain.Block, error) {
...
func (b *EXChainRPC) GetBlockInfo(hash string) (*bchain.BlockInfo, error) {
	raw, err := b.getBlockRaw(hash, 0, false)
	if err != nil {
		return nil, err
	}
...
func (b *EXChainRPC) GetTransactionForMempool(txid string) (*bchain.Tx, error) {
	return b.GetTransaction(txid)
}

// GetTransaction returns a transaction by the transaction ID.
func (b *EXChainRPC) GetTransaction(txid string) (*bchain.Tx, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
...
func (b *EXChainRPC) GetTransactionSpecific(tx *bchain.Tx) (json.RawMessage, error) {
...
func (b *EXChainRPC) GetMempoolTransactions() ([]string, error) {
...
func (b *EXChainRPC) EstimateFee(blocks int) (big.Int, error) {
	return b.EstimateSmartFee(blocks, true)
}

// EstimateSmartFee returns fee estimation
func (b *EXChainRPC) EstimateSmartFee(blocks int, conservative bool) (big.Int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
	var r big.Int
...
func (b *EXChainRPC) EthereumTypeEstimateGas(params map[string]interface{}) (uint64, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
...
func (b *EXChainRPC) SendRawTransaction(hex string) (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
	var raw json.RawMessage
...
func (b *EXChainRPC) EthereumTypeGetBalance(addrDesc bchain.AddressDescriptor) (*big.Int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
...
func (b *EXChainRPC) EthereumTypeGetNonce(addrDesc bchain.AddressDescriptor) (uint64, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
...
func (b *EXChainRPC) GetChainParser() bchain.BlockChainParser {
	return b.Parser
}
