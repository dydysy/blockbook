...
import (
	...
	"github.com/dydysy/blockbook/tests/dbtestdata"
)
...
func TestEthereumParser_PackTx(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("OKExChainParser.PackTx() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			h := hex.EncodeToString(got)
			if !reflect.DeepEqual(h, tt.want) {
				t.Errorf("OKExChainParser.PackTx() = %v, want %v", h, tt.want)
			}
		})
	}
}
...
func TestEthereumParser_UnpackTx(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("OKExChainParser.UnpackTx() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			...
			if fmt.Sprint(gc) != fmt.Sprint(wc) {
				...
				t.Errorf("OKExChainParser.UnpackTx() gc got = %+v, want %+v", gc, wc)
			}
			if !reflect.DeepEqual(gs.Tx, ws.Tx) {
				t.Errorf("OKExChainParser.UnpackTx() gs.Tx got = %+v, want %+v", gs.Tx, ws.Tx)
			}
			if !reflect.DeepEqual(gs.Receipt, ws.Receipt) {
				t.Errorf("OKExChainParser.UnpackTx() gs.Receipt got = %+v, want %+v", gs.Receipt, ws.Receipt)
			}
			if got1 != tt.want1 {
				t.Errorf("OKExChainParser.UnpackTx() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
...
func TestEthereumParser_GetEthereumTxData(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got.Data != tt.want {
				t.Errorf("OKExChainParser.GetEthereumTxData() = %v, want %v", got.Data, tt.want)
			}
		})
	}
}
