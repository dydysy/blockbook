...
// OKExChainParser handle
type OKExChainParser struct {
	*bchain.BaseParser
}

// NewOKExChainParser returns new OKExChainParser instance
func NewOKExChainParser(b int) *OKExChainParser {
	return &OKExChainParser{&bchain.BaseParser{
...
func (p *OKExChainParser) ethTxToTx(tx *rpcTransaction, receipt *rpcReceipt, blocktime int64, confirmations uint32, fixEIP55 bool) (*bchain.Tx, error) {
	txid := tx.Hash
	var (
		fa, ta []string
		err    error
...
func (p *OKExChainParser) GetAddrDescFromVout(output *bchain.Vout) (bchain.AddressDescriptor, error) {
...
func (p *OKExChainParser) GetAddrDescFromAddress(address string) (bchain.AddressDescriptor, error) {
...
func (p *OKExChainParser) GetAddressesFromAddrDesc(addrDesc bchain.AddressDescriptor) ([]string, bool, error) {
	return []string{EIP55Address(addrDesc)}, true, nil
}

// GetScriptFromAddrDesc returns output script for given address descriptor
func (p *OKExChainParser) GetScriptFromAddrDesc(addrDesc bchain.AddressDescriptor) ([]byte, error) {
	return addrDesc, nil
}
...
func (p *OKExChainParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	var err error
	var n uint64
...
func (p *OKExChainParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
...
func (p *OKExChainParser) PackedTxidLen() int {
	return 32
}

// PackTxid packs txid to byte array
func (p *OKExChainParser) PackTxid(txid string) ([]byte, error) {
	if has0xPrefix(txid) {
		txid = txid[2:]
	}
...
func (p *OKExChainParser) UnpackTxid(buf []byte) (string, error) {
	return hexutil.Encode(buf), nil
}

// PackBlockHash packs block hash to byte array
func (p *OKExChainParser) PackBlockHash(hash string) ([]byte, error) {
	if has0xPrefix(hash) {
		hash = hash[2:]
	}
...
func (p *OKExChainParser) UnpackBlockHash(buf []byte) (string, error) {
	return hexutil.Encode(buf), nil
}

// GetChainType returns EthereumType
func (p *OKExChainParser) GetChainType() bchain.ChainType {
...
func (p *OKExChainParser) EthereumTypeGetErc20FromTx(tx *bchain.Tx) ([]bchain.Erc20Transfer, error) {
